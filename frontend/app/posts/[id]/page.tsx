...
type ApiPost = Parameters<typeof mapPost>[0] & {
  feeds?: Array<{ id: string; title: string; code: string }>;
};
...
export default function PostReaderPage() {
      ...
      try {
        ...
        if (!cancelled) {
          setPost({
            ...
            channelNames: payload.data.feeds?.map((feed) => feed.title) ?? [],
          });
          setState("ready");
        }
      } catch {
  ...
  if (state === "loading") {
    return (
      <main className="mx-auto max-w-3xl py-16 text-center text-[var(--text-muted)]">
        Loading post…
      </main>
    );
  }
  if (state === "not-found") {
    return (
      <main className="mx-auto max-w-3xl py-16 text-center">
        ...
        <Link className="mt-5 inline-block font-bold text-[var(--primary)]" href="/posts">
          Back to posts
        </Link>
      </main>
    );
  ...
  if (state === "error") {
    return (
      <main className="mx-auto max-w-3xl py-16 text-center">
        <h1 className="text-3xl font-bold">Post unavailable</h1>
        <p className="muted mt-3">The content API could not load this post.</p>
      </main>
    );
  }
  if (!post) return null;
  ...
  return (
    <main className="mx-auto max-w-3xl">
      <article className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[var(--shadow)] sm:p-9">
        ...
        {post.channelNames.length > 0 && (
          <div className="mt-5">
            <h2 className="text-sm font-bold">Channels</h2>
            <div className="mt-2 flex flex-wrap gap-2">
              {post.channelNames.map((channel) => (
                <span
                  key={channel}
                  className="rounded-full bg-[var(--primary-soft)] px-3 py-1 text-sm font-semibold text-[var(--primary)]"
                >
                  {channel}
                </span>
              ))}
        ...
        {post.body.split("\n").map((paragraph, index) => (
          <p key={index} className="mt-5 leading-8">
            {paragraph}
          </p>
        ))}
        {post.externalLink && (
          <a
            className="mt-7 inline-block font-bold text-[var(--primary)] hover:underline"
      ...
      <Link
        className="mt-6 inline-block font-bold text-[var(--primary)] hover:underline"
        href="/posts"
      >
        ← Back to posts
      </Link>
