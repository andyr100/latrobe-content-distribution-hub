  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-30 border-b border-[var(--border)] bg-[color-mix(in_srgb,var(--bg)_78%,transparent)] backdrop-blur-xl">
        <div className="mx-auto flex h-[4.5rem] max-w-[1600px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <div className="grid size-10 place-items-center rounded-xl bg-[var(--primary)] text-sm font-black text-white shadow-lg">
              LT
            </div>
          ...
          <div className="mr-14 flex items-center gap-2 lg:mr-0">
            <span className="hidden rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 text-xs font-semibold text-[var(--text-muted)] sm:inline">
              API connected
            </span>
            <ThemeToggle />
            <UserProfileChip />
      ...
      <div className="mx-auto flex max-w-[1600px]">
        ...
        <main
          id="main-content"
          className="min-h-[calc(100vh-8.5rem)] min-w-0 flex-1 px-4 py-8 sm:px-6 lg:px-10"
        >
          {children}
        </main>
      </div>
