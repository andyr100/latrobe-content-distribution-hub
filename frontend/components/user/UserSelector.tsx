...
export function UserSelector() {
  const { userSelectorOpen, selectedUser, users, usersLoading, usersError, selectUser, closeUserSelector } = useSession();
  ...
  return (
    <Modal open={userSelectorOpen} title="Select your profile" description="Choose a mock user to continue. This selection is session-only and does not represent authentication." onClose={closeUserSelector} locked={!selectedUser} size="sm">
      <div className="space-y-2" role="radiogroup" aria-label="Available users">
        {usersLoading && <p className="muted py-6 text-center text-sm" role="status">Loading users from the API…</p>}
        {usersError && <p className="rounded-xl border border-[var(--danger)] p-3 text-sm text-[var(--danger)]" role="alert">{usersError}. Confirm the API is running on port 4000.</p>}
        {users.map((user) => {
