...
The parameterised plan and staged PowerShell runner are in `tests/jmeter/`. It sends `X-Client-Id: jmeter-${threadNum}` and `X-Client-Source: jmeter`, so every stage becomes visible in the Assessment 3 dashboard and Reports page.
