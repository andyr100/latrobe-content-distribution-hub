...
type Action = { type: "hydrate"; channels: Channel[]; posts: InternalPost[] } | { type: "addChannel"; channel: Channel } | { type: "deleteChannel"; id: string } | { type: "addPost"; post: InternalPost } | { type: "deletePost"; id: string } | { type: "reset" };
function reducer(state: State, action: Action): State {
  ...
  if (action.type === "deletePost") return { ...state, posts: state.posts.filter((post) => post.id !== action.id) };
  return state;
}
...
type Value = State & { addChannel: (channel: Channel) => void; deleteChannel: (id: string) => void; addPost: (post: InternalPost) => void; deletePost: (id: string) => void; resetContent: () => void };
...
export function ContentProvider({ children }: { children: ReactNode }) {
  ...
  return <ContentContext.Provider value={{ ...state, addChannel: (channel) => dispatch({ type: "addChannel", channel }), deleteChannel: (id) => dispatch({ type: "deleteChannel", id }), addPost: (post) => dispatch({ type: "addPost", post }), deletePost: (id) => dispatch({ type: "deletePost", id }), resetContent: () => dispatch({ type: "reset" }) }}>{children}</ContentContext.Provider>;
}
